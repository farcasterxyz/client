...
function ProtocolConnectedAddresses({
  ...
  const onSetAsPrimary = useCallback(
    async (verification: ApiVerification) => {
      ...
      try {
        // `mutate` returns void, so awaiting it resolves immediately and
        // swallows failures — the success toast then fires even when the PUT
        // was rejected, which is how users end up believing their primary
        // address changed when it did not. `mutateAsync` actually rejects.
        await putVerification.mutateAsync(verification);
      } catch (error) {
        trackError(error);
        toast.show(`Failed to update primary ${protocolName} address`, {
          type: 'danger',
          placement: 'bottom',
        });
        return;
