import { type InFlightJoinRef, runDedupedJoin } from '../spaceJoinInFlight';
...
describe('runDedupedJoin (farcaster-mobile)', () => {
  test('shares one in-flight join between concurrent calls for the same room', async () => {
    const ref = createRef();
    const first = deferred<boolean>();
    const start = jest.fn().mockReturnValue(first.promise);
