...
// Single NetInfo callback drives both sinks, in this order deliberately.
// `setOnline` notifies React Query's listeners synchronously, which is where
// paused fetches resume — so the api client's `isOffline` must already be
// current by then. Registering a second, independent NetInfo listener (as
// apiClient.ts previously did) does not guarantee that: NetInfo dispatches to
// its subscriber Set in insertion order, and this module is imported before
// MobileFarcasterApiClientProvider in App.tsx, so `setOnline(true)` would have
// run first and the resumed attempt would still see the stale `isOffline:true`
// and burn a retry on an instant 'Offline' throw (NEYN-13137).
onlineManager.setEventListener((setOnline) => {
  return NetInfo.addEventListener((netInfo) => {
    const isOnline = computeIsOnline(netInfo);
    apiClient.updateOptions({ isOffline: !isOnline });
    setOnline(isOnline);
  });
});
