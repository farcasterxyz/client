...
import { type InFlightJoin, runDedupedJoin } from './spaceJoinInFlight';
...
const SpaceProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const joinInFlightRef = useRef<InFlightJoin | null>(null);
      ...
      if (canPublish) {
        try {
          ...
          if (isMicPermissionError(err)) {
            ...
            // Not a permission problem (e.g. the session dropped while the
            // publication was being created). Leave the denied banner alone;
            // the next mic tap goes through the refresh path.
  ...
  // Plain joins for a room already being joined share the in-flight attempt.
  // The scheduled "Go live" handler and the live screen's auto-join effect can
  // both call join() within the same second; two LiveKit sessions for one
  // identity disconnect each other and the backend then kicks the survivor.
  // Deliberate session refreshes (explicit spaceSessionId / joinResultOverride)
  // are never deduped.
  const join = useCallback(
    (
      ...
      if (options?.joinResultOverride || options?.spaceSessionId) {
        return joinInternal(roomId, entrySource, options);
      }
      return runDedupedJoin({
        ref: joinInFlightRef,
        roomId,
        generation: joinGenerationRef.current,
        start: () => joinInternal(roomId, entrySource, options),
      });
    },
    [joinInternal],
  );
