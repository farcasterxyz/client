...
const useMixedFeedItems = ({
  ...
  // Returns the React Query result so callers can tell a failed pagination
  // from a successful one without inspecting query-level state (which is also
  // set by first-page loads and pull-to-refresh).
  ...
  // User-initiated retry for a failed or wedged pagination. Cancels the
  // query first: with networkMode 'offlineFirst', a fetchNextPage whose
  // retry paused while offline never settles its promise on its own
  // (NEYN-13137) — cancelQueries settles it so the fresh fetch below always
  // starts from a clean state instead of being deduped into the stuck one.
  const retryFetchNextPage = useCallback(async () => {
