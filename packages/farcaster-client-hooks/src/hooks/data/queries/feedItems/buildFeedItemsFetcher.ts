  ...
  async ({ pageParam }: { pageParam?: FeedItemsPageParam }) => {
    ...
    try {
      const response = await apiClient.getFeedItems(
        ...
        // retryLimit: 0 — React Query owns retries for this query (see
        // feedItemsDefaultQueryOptions). getFeedItems is a POST, so without
        // this the client-level fetchWithRetry loop nests inside React
        // Query's, turning one failed pagination into up to 12 requests and
        // ~40s of spinner before the error surfaces (NEYN-13137).
        { timeout: DEFAULT_TIMEOUT_FEED_ITEMS, retryLimit: 0 },
      );
