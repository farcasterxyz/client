...
abstract class AbstractFarcasterApiClient {
  ...
  private async fetch<T>(
    ...
    // Decay only applies to requests running on the generic default timeout.
    // An explicit per-request `timeout` (DEFAULT_TIMEOUT_FEED_ITEMS etc.) is
    // already tuned to the endpoint's P99; multiplying it down guarantees
    // failure for tail-latency users. Worse, the decay key is url+params, so
    // for POST endpoints (params undefined, body not in the key) every call
    // to the endpoint — across feeds, retries, and pull-to-refresh — shares
    // one counter, and a few calls per minute pinned /v2/feed-items at a
    // 1.5s timeout no slow connection could meet (NEYN-13137).
    const timeoutDecayFactor =
      this.timeoutRetryDecayManager && timeout === undefined
