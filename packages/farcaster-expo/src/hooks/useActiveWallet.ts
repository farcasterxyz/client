...
export function useActiveWallet(): ActiveWalletSelection {
  ...
  // True only once we have actually fetched the wallet list.
  //
  // `secondaryWalletsEnabled` is derived from the `secondary-wallets` feature
  // flag, which lives on the authenticatedUser query. That query is not
  // persisted, so on every cold start the flag reads `false` for the first
  // render pass and this query stays disabled — leaving `wallets` empty for
  // reasons that have nothing to do with the user owning a secondary wallet.
  // Anything that decides "this user has no secondary wallet" must gate on
  // this, or it will draw that conclusion on every single app launch.
  const walletsLoaded = secondaryWalletsEnabled && data !== undefined;
  ...
  // Until the wallet list is known (feature off, or still loading) we report
  // `primary` rather than pointing at a secondary wallet we cannot resolve.
  // This is derived only — the stored preference is deliberately left alone, so
  // the user's choice comes back as soon as the real data lands.
  const activeNamespace = React.useMemo<ActiveWalletNamespace>(() => {
    if (!walletsLoaded) {
      return PRIMARY_NAMESPACE;
    }
    return normalizeNamespace(storedNamespace, wallets);
  }, [walletsLoaded, storedNamespace, wallets]);
  ...
  //
  // Gated on `walletsLoaded`: this is the only place that destroys the stored
  // preference, so it must never run against an empty list we haven't fetched.
  React.useEffect(() => {
    if (!walletsLoaded) {
      return;
    }
    ...
    if (
      ...
      setStoredNamespace(normalizeNamespace(storedNamespace, wallets));
      return;
    }
    if (
      storedNamespace === SECONDARY_NAMESPACE &&
