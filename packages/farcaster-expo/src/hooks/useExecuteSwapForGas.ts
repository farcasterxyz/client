...
export async function executeQuoteAsync({
  ...
  // Find the permit and trade permission items. 0x returns the gasless approval
  // as a plain EIP-2612 `permit` or USDC's ERC-7597 `erc7597Permit` — both are
  // signed identically (EIP-712) and handed straight back to 0x.
  const isApprovalSignature = (data: {
    type: string;
  }): data is ApiSwapStepPermit2Signature =>
    data.type === 'permit' || data.type === 'erc7597Permit';

  const permit = items.find(
    (item) => item.type === 'signature' && isApprovalSignature(item.data),
  );
  ...
  const tradeMsg =
    ...
    tradePermission.data.type === 'settler_metatransaction'
      ...
      : {};
  const permitMsg =
    permit?.type === 'signature' && isApprovalSignature(permit.data)
  ...
  if (
    ...
    isApprovalSignature(permit.data)
  ) {
