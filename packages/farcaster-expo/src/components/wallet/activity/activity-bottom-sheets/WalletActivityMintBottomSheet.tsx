...
import { ActivityTokenIcon, getTypeText } from '../WalletActivityItem';
...
export function WalletActivityMintBottomSheet({
  item,
}: {
  ...
  // For mint, get the IN state change (tokens coming into wallet)
  ...
  const fromRow = useGetAddressRow(item, stateChange, item.chain, 'from');

  // Use the same logic as ActivityTypeStatus to determine the text
  const activityTypeText = useMemo(() => {
    return getTypeText(item.type, item.status);
  ...
  const tableRows: TableRow[] = useMemo(() => {
    ...
    return rows;
  }, [fromRow, convertedForGasFeesRow, miniAppRow, stateChange, PurpleHashRow]);
